...
func GenerateAll(ctx context.Context, outPath string, namespaces *pb.Namespaces) (err error) {
	...
	header := generatedCodeHeader(templateName)
	if _, err := builderFile.WriteString(header); err != nil {
		return fmt.Errorf("failed to write header: %w", err)
	}

	if err := templs.ExecuteTemplate(builderFile, templateName, nil); err != nil {
...
func generateElement(_ context.Context, pkgPath string, globalAttributes []*pb.Attribute, ns *pb.Namespace, element *pb.Element) error {
	...
	if err := templs.ExecuteTemplate(f, templateName, map[string]any{
		"Namespace": ns,
		"Element":   element,
	}); err != nil {
		return fmt.Errorf("failed to execute template: %w", err)
...
func generatedCodeHeader(templateName string) string {
	return fmt.Sprintf(`// Code generated by speckles. DO NOT EDIT.
// Source: generator/templates/%s
// Generated: %s

`, templateName, time.Now().Format(time.RFC3339))
}
